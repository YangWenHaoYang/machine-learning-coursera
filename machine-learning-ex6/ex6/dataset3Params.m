...
score = zeros(length(C_try) * length(sigma_try), 3);
for i = 1 : length(C_try)
	...
	for j = 1:length(sigma_try)
		...
		predictions = svmPredict(model, Xval);
		score((i-1)*length(sigma_try)+j,:) = [Ci sigmai mean(double(predictions~=yval))];
	end
end

[M I] = min(score);
min_score_index = I(3);
C = score(min_score_index, 1);
sigma = score(min_score_index, 2);
